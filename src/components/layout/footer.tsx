import Link from "next/link";
import { SITE_CONFIG } from "@/lib/constants";
import { MapPin, Phone, Mail } from "lucide-react";

const footerLinks = {
  shop: [
    { label: "All Products", href: "/products" },
    { label: "Nuts", href: "/products?category=nuts" },
    { label: "Dried Fruits", href: "/products?category=dried-fruits" },
    { label: "Seeds", href: "/products?category=seeds" },
  ],
  company: [
    { label: "About / Origins", href: "/about" },
    { label: "Journal", href: "/blog" },
    { label: "Shipping & Rates", href: "/shipping" },
    { label: "Contact", href: "/contact" },
  ],
  support: [
    { label: "Leave a Review", href: "/feedback" },
    { label: "Ask on WhatsApp", href: "https://wa.me/923001234567", external: true },
    { label: "Bulk Orders", href: "/contact" },
  ],
};

export function Footer() {
  return (
    <footer style={{ background: "#1A1512", color: "#EDE7DC" }}>
      {/* Main footer */}
      <div
        style={{ maxWidth: "1400px", margin: "0 auto", padding: "64px 28px 56px" }}
      >
        <div
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-12"
        >
          {/* Brand */}
          <div className="lg:col-span-2">
            <Link href="/" className="inline-block">
              <span style={{ fontSize: "25px", fontWeight: 800, letterSpacing: "-.05em", color: "#fff" }}>
                QAAQ
              </span>
              <span
                className="block"
                style={{
                  fontSize: "8px",
                  letterSpacing: ".42em",
                  textTransform: "uppercase",
                  color: "#7C7268",
                  fontWeight: 600,
                  marginTop: "3px",
                }}
              >
                Dry Fruits
              </span>
            </Link>
            <p
              style={{
                marginTop: "20px",
                fontSize: "14px",
                lineHeight: 1.7,
                color: "rgba(237,231,220,.5)",
                maxWidth: "300px",
              }}
            >
              Single-origin dry fruits from five valleys. Packed to order, lab-tested every batch, shipped within 48 hours.
            </p>
            {/* WhatsApp CTA */}
            <a
              href="https://wa.me/923001234567"
              target="_blank"
              rel="noopener noreferrer"
              className="qaaq-press inline-flex items-center gap-[10px]"
              style={{
                marginTop: "24px",
                height: "42px",
                padding: "0 20px",
                background: "#1FA855",
                color: "#fff",
                fontSize: "13px",
                fontWeight: 600,
                borderRadius: "2px",
              }}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="#fff">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
              </svg>
              Order on WhatsApp
            </a>
          </div>

          {/* Shop */}
          <div>
            <h3
              style={{
                fontSize: "10.5px",
                letterSpacing: ".2em",
                textTransform: "uppercase",
                fontWeight: 700,
                color: "rgba(237,231,220,.4)",
                marginBottom: "20px",
              }}
            >
              Shop
            </h3>
            <ul className="space-y-3">
              {footerLinks.shop.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="qaaq-und"
                    style={{ fontSize: "13.5px", color: "rgba(237,231,220,.65)" }}
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Company */}
          <div>
            <h3
              style={{
                fontSize: "10.5px",
                letterSpacing: ".2em",
                textTransform: "uppercase",
                fontWeight: 700,
                color: "rgba(237,231,220,.4)",
                marginBottom: "20px",
              }}
            >
              Company
            </h3>
            <ul className="space-y-3">
              {footerLinks.company.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="qaaq-und"
                    style={{ fontSize: "13.5px", color: "rgba(237,231,220,.65)" }}
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3
              style={{
                fontSize: "10.5px",
                letterSpacing: ".2em",
                textTransform: "uppercase",
                fontWeight: 700,
                color: "rgba(237,231,220,.4)",
                marginBottom: "20px",
              }}
            >
              Contact
            </h3>
            <ul className="space-y-3">
              <li className="flex items-start gap-[10px]" style={{ fontSize: "13.5px", color: "rgba(237,231,220,.65)" }}>
                <MapPin style={{ width: "14px", height: "14px", marginTop: "2px", flexShrink: 0 }} />
                <span>{SITE_CONFIG.address}</span>
              </li>
              <li>
                <a
                  href={`tel:${SITE_CONFIG.phone}`}
                  className="flex items-center gap-[10px]"
                  style={{ fontSize: "13.5px", color: "rgba(237,231,220,.65)" }}
                >
                  <Phone style={{ width: "14px", height: "14px", flexShrink: 0 }} />
                  {SITE_CONFIG.phone}
                </a>
              </li>
              <li>
                <a
                  href={`mailto:${SITE_CONFIG.email}`}
                  className="flex items-center gap-[10px]"
                  style={{ fontSize: "13.5px", color: "rgba(237,231,220,.65)" }}
                >
                  <Mail style={{ width: "14px", height: "14px", flexShrink: 0 }} />
                  {SITE_CONFIG.email}
                </a>
              </li>
            </ul>
          </div>
        </div>
      </div>

      {/* Bottom bar */}
      <div style={{ borderTop: "1px solid rgba(255,255,255,.08)" }}>
        <div
          className="flex flex-col sm:flex-row items-center justify-between gap-4"
          style={{ maxWidth: "1400px", margin: "0 auto", padding: "20px 28px" }}
        >
          <p style={{ fontSize: "11.5px", color: "rgba(237,231,220,.3)" }}>
            &copy; {new Date().getFullYear()} QAAQ. All rights reserved.
          </p>
          <div className="flex items-center gap-6">
            {SITE_CONFIG.social.instagram && (
              <a
                href={SITE_CONFIG.social.instagram}
                target="_blank"
                rel="noopener noreferrer"
                style={{
                  fontSize: "11px",
                  letterSpacing: ".14em",
                  textTransform: "uppercase",
                  color: "rgba(237,231,220,.35)",
                }}
              >
                Instagram
              </a>
            )}
            {SITE_CONFIG.social.facebook && (
              <a
                href={SITE_CONFIG.social.facebook}
                target="_blank"
                rel="noopener noreferrer"
                style={{
                  fontSize: "11px",
                  letterSpacing: ".14em",
                  textTransform: "uppercase",
                  color: "rgba(237,231,220,.35)",
                }}
              >
                Facebook
              </a>
            )}
          </div>
        </div>
      </div>
    </footer>
  );
}
